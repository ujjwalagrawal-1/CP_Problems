...
void ujjwal(){
    ...
    ll cnt = 1e18;
    rep(i,x){
        X.erase(X.find(veci[i][0]));
        Y.erase(Y.find(veci[i][1]));
        ll minx = *X.begin();
        ll maxx = *X.rbegin();
        ll miny = *Y.begin();
        ll maxy = *Y.rbegin();
        bool fl = 0;
        if((maxx - minx + 1)*(maxy - miny + 1)*1ll > x-1){
            cnt = min(cnt,1ll*(maxx - minx + 1)*(maxy - miny + 1));
        }
        else{
            cnt = min(cnt,x-1 + 1ll*min(maxx-minx+1 < ll(1e9)? maxx - minx + 1 : ll(1e9),maxy - miny+1 < ll(1e9) ?  maxy - miny+1 : ll(1e9)));
        }
        X.insert(veci[i][0]);
        Y.insert(veci[i][1]);
    }
