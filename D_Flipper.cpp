...
void ujjwal(){
    ...
    if(maxiind == 0){
        // check for the next minimum fun
        if(veci.back() == x-1){
            os(veci.back());
            rep(i,x-1){
                os(veci[i]);
        ...
        // check left - left part with left - right; and  simillarly change the pointer
        // cy; 
        ll l = x-1;
        rrep(i,x-1){
            if(veci[i] > veci[0]){
