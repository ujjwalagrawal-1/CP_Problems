...
void ujjwal(){
    // Code Starts Here
    inll(x);
    dvecl veci(x,vecl(x,0));
    cin>>veci;
    dvecl temp(x,vecl(x,1));
    queue<pll> qu;
    rep(i,x){
        rep(j,x){
            if(veci[i][j] == 0){
    ...
    while(!qu.empty()){
        ...
        rep(i,4) {
            ...
            if(nx >= 0 && ny >= 0 && nx < x &&  ny < x && veci[nx][ny] == 1){
                if(ans[px][py] + 1 < ans[nx][ny]){
                    ans[nx][ny] = ans[px][py]+1;
                    qu.push({nx,ny});
                }
            }
        }
