...
void ujjwal(){
    ...
    cin>>veci;
    vector<dvecl> dp(n+2,dvecl(m+2,vecl(3,LLONG_MAX)));

    queue<array<ll,3>> qu;

    ll ex,ey;
    queue<array<ll,3>> squ;
    for(int i = 0; i<n;i++){
        for(int j = 0;j<m;j++){
            if(veci[i][j] == 'S'){
                qu.push({i,j,0});
    ...
    while(!qu.empty()){
        ...
        rep(i,2){
            ll nx = node[0] + dx[i];
            ll ny = node[1] + dy[i];
            ll nd = dp[node[0]][node[1]][node[2]] + 1;
            if(nx >= n || nx <0 || ny >= m || ny < 0 ||veci[nx][ny] == '#')C;
            if(nd < dp[nx][ny][node[2] == 0 ? 1 : 0]){
                dp[nx][ny][node[2] == 0 ? 1 : 0] = nd;
                qu.push({nx,ny,node[2] == 0 ? 1 : 0});
            }
        }
    }
    if(dp[ex][ey][0] == LLONG_MAX && dp[ex][ey][1] == LLONG_MAX){
        cm;
        rtn;
    }
    else{
        out(min(dp[ex][ey][0],dp[ex][ey][1]));
    }
}
