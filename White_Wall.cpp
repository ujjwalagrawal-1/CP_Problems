...
ll f(string st,char a,char b,char c){
    ll cnt = 0;
    rep(i,sz(st)){
        if(i%3 == 0 && st[i] != a){
            cnt++;
        }
        else if(i%3 == 1 && st[i] != b){
            cnt++;
        }
        else if(i%3 == 2 && st[i] != c){
            cnt++;
        }
    }
    return cnt;
...
void ujjwal(){
    ...
    vecl veci(3);
    string e = "RGB";
    ll ans = min({f(st,e[0],e[1],e[2]),f(st,e[0],e[2],e[1]),f(st,e[1],e[0],e[2]),f(st,e[1],e[2],e[0]),f(st,e[2],e[1],e[0]),f(st,e[2],e[0],e[1])});
    out(ans);
}
