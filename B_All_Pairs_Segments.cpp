...
void solve()
{
    ...
    rep(i,x){
        ...
        mp[t1]++;
        if(i > 0){
            // Beacause i is One Step Ahead checking is Done 
            // when i = 1 it is checking for open interval (veci[0] , veci[1]); so number of elements before it 
            // is i*(n-i); 
            ll t2 = (i)*(x-i);
            mp[t2] += ((veci[i] - veci[i-1]) - 1);
        }
    }
    vecl ans;
    rep(i,y){
