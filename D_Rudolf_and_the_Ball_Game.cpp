...
// void prepare_tree(vector<int>& veci, vector<char>& ch, int i, node*& root, int n) {
//     if (root == NULL) {
//         return;
//     }
//     if (i == veci.size()) {
...
//     if (ch[i] == '?') {
//         root->left = new node((root->val - veci[i] <= 0 ? root->val - veci[i] + n : root->val - veci[i]));
//         root->right = new node((root->val + veci[i] > n ? root->val + veci[i] - n : root->val + veci[i]));
//     } else if (ch[i] == '0') {
//         root->right = new node((root->val + veci[i] > n ? root->val + veci[i] - n : root->val + veci[i]));
//     } else {
//         root->left = new node((root->val - veci[i] <= 0 ? root->val - veci[i] + n : root->val - veci[i]));
...
//     prepare_tree(veci, ch, i + 1, root->left, n);
//     prepare_tree(veci, ch, i + 1, root->right, n);
// }
...
void solve() {
    ...
    // prepare_tree(veci, ch, 0, newnode, n);
    // out(st.size());
}
