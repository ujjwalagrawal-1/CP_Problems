...
void ujjwal(){
    ...
    cin>>a>>b;
    vecl ks(a);
    dvecl veci(b,vecl(3,0));
    cin>>ks>>veci;
    // debug(ks);
    // debug(veci);
    sort(all(ks));
    rep(i,b){
        if(4*veci[i][0]*veci[i][2]*1ll < 0){
            cn;
            C;
        }
        double val = veci[i][1] - sqrtl(4*veci[i][0]*veci[i][2]*1ll);
        auto it = upper_bound(all(ks),val);
        // out("val  is "<<val);
        if(it != ks.end() && *it < veci[i][1] + sqrtl(4*veci[i][0]*veci[i][2]*1ll)){
            cy;
            out(*it);
        }
        else{
