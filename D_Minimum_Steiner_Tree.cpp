...
ll modinv(ll p,ll q){ll ex;ex=M-2;
\ (ex) {if (ex & 1) {p = (p * q) % M;}q = (q * q) % M;ex>>= 1;}return p;}
...
ll f(set<ll>&st,vecl adj[],ll node,ll pnode){
    ll ans = st.count(node);
    for(auto itr : adj[node]){
        if(pnode == itr){
            C;
        }
        ans += f(st,adj,itr,node);
    }
    if(ans != 0 && !st.count(node)){
        return ans+1;
    }
    return ans;
}
