...
#define mploop(itr,mp) for(auto itr=mp.begin();itr!=mp.end();itr++)
#define mprloop(itr,mp) for(auto itr=mp.rbegin();itr!=mp.rend();itr++)
...
void ujjwal(){
    // Code Starts Here
    inll(x);
    vecl veci(x);cin>>veci;
    sort(all(veci));
    vecl temp;
    rep(i,x){
        if(i+1 < x && veci[i] == veci[i+1]){
            temp.pb(veci[i]);
            i++;
        }
    }
    if(temp.size() < 4){
        ...
        cy;
        out(temp[0]<<" "<<temp[1]<<" "<<temp[0]<<" "<<temp[sz(temp)-1]<<" "<<temp[sz(temp)-2]<<" "<<temp[1]<<" "<<temp[sz(temp)-2]<<" "<<temp[sz(temp)-1])
    }
}
