...
void ujjwal(){
    ...
    cin>>veci;
    ll ans = LLONG_MAX;
    ll sumeven = veci[0],sumodd = veci[1];
    ll minofeven =veci[0],minofodd = veci[1];
    ll cntodd = 1,cnteven = 1;
    rep(i,n){
        if(i%2 == 0){
            minofeven = min(minofeven,veci[i]);
            if(i != 0){
                cnteven++;
                sumeven += veci[i];
            }
            ans = min(ans,(sumeven - minofeven) + (sumodd - minofodd) + 1ll*minofeven*(n - (cnteven-1)) + 1ll*minofodd*(n - (cntodd-1)));
        }
        else{
            minofodd = min(minofodd,veci[i]);
            if(i != 1){
                cntodd++;
                sumodd += veci[i];
            }
