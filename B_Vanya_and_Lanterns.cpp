...
//     cin>>veci;
//     sort(all(veci));
//     veci.insert(veci.begin(),0);
//     veci.insert(veci.begin()+veci.size(),y);
//     // out(veci);
//     ll maxi = INT_MIN;
//     feach(i,1,x-1,1){
//         maxi = max(maxi,veci[i+1] - veci[i]);
//     }
//     float ans = (float)((float)maxi/(float)2);
//     if(ans < veci[1] - veci[0] ||  ans < veci[sz(veci)-1] - veci[sz(veci)-2]){
//         maxi = max({ans,(float)(veci[sz(veci)-1] - veci[sz(veci)-2]),(float)(veci[1] -veci[0])});
//         stdcout.precision(20);
//         out(float(maxi));rtn;
//     }
//     // out(maxi);
//     cout.precision(20);
//     out((float)((float)maxi/(float)2.00));
// }   
