...
void ujjwal(){
    ...
    // for(int i = 0;i<veci.size();i++){
    //     for(auto itr : veci[i])cout<<itr;
    //     cout<<endl;
    // }
    // dp[i][j] = tell me the total path from 0,0 to i,j;

    // base case calculation
    // what is the base case in this Sitiuations 
    dvecl dp(x,vecl(x,0));
    ll fla = 1,flb = 1;
    for(int i = 0;i<x;i++){
        if(veci[0][i] != '*' && fla)dp[0][i] = 1;
        else fla = 0;
        if(veci[i][0] != '*' && flb)dp[i][0] = 1;
        else flb = 0;
    }
    for(int i = 1;i<x;i++){
        for(int j = 1;j<x;j++){
