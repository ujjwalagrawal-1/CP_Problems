...
void ujjwal(){
    ...
    while(i<x){
        if(a > b){
            swap(a,b);
            // b SHould be the Highest;
        }
        if(a >= veci[i]){
            // replace with the SMallest among two
            a = veci[i];
        }
        else if(b >= veci[i]){
            // if not replacable with the smallest rplace with the highest among them
            b = veci[i];
        }
        else{
            // if not replaceable with both repalce with the smallest
            a = veci[i];
            cnt++;
        }
        i++;
