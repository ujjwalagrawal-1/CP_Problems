...
int f(const string& st, int k) {
    unordered_map<int, int> mp;
    int sum = 0, ans = 0;
    mp[sum] = 1;
    for (char it : st) {
        sum += (it == '0' ? -1 : 1); // Correctly update sum based on character
        int find = sum - k;
...
void solve() {
    string str;
    cin >> str; // Assuming input is provided via standard input
    long long ans = f(str, 0);
