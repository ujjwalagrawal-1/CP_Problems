...
ll fun(vector<string> &veci, ll i, ll j)
{
    int n = veci.size();
    int m = veci[0].size();
    if (i < 0 || i == n || j < 0 || j == m)
    ...
    if (veci[i][j] == '1')
    {
        ...
        for(int k = 0;k<4;k++)
        {
            ...
            mini = min(mini, fun(veci, i + veci[k][0], j + veci[k][1]) + 1);
        }
        veci[i][j] == '1';
    }
    else if(veci[i][j] == '0')
    {
        ...
        for(int k = 0;k<4;k++)
        {
            mini = min(mini, fun (veci, i + veci[k][0], j + veci[k][1]));
        }
        veci[i][j] = '0';
    }
