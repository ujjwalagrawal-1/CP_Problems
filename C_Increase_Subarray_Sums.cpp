...
void solve()
{
    ...
    rep(i,x){
        ...
        feach(j,i,x,1){
            ...
            if(!compute[j - i + 1]){
                ...
                compute[j-i+1] = max(1ll*compute[j-i+1],sum);
            }
            compute[j-i+1] = max(1ll*compute[j-i+1],sum);
        }
    }
    ...
    // rtn;
    // for(auto itr : mp){
    //     out("itr.fir  == " << itr.fir << "   itr.sec "<< itr.sec);
    // }
    // rtn;
    vecl ans;
    ans.push_back(0);
    rep(j,x+1){
        ll maxi = 0;
        for(int i = j;i<sz(compute);i++){
            // out("compute value is " <<compute[i]);
            maxi = max(maxi,compute[i]+ k*j);
        }
        ans.push_back(max(maxi,ans.back()));
    }
    // out("ans array is   ->");
    feach(i,1,sz(ans),1){
        os(ans[i]);
