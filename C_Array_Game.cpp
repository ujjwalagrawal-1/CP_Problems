...
void solve()
{
    ...
    if(k == 1){
        ...
        rep(i,x-1){
            feach(j,i+1,x,1){
                ll sub = veci[j] - veci[i];
                auto it = lower_bound(all(veci),sub);
                if(it == veci.end()){
                  // cy;
                    ans = min(ans,abs(veci.back() - sub));
                    //   cn;
                }
                else{
                    ans = min(ans,abs(veci[it - veci.begin()] - sub));
                    if(it != veci.begin())
                    ans = min(ans,abs(veci[prev(it) - veci.begin()] - sub));
                }
                ans = min(ans,sub);
                ans = min(ans,veci[j]);
            }
