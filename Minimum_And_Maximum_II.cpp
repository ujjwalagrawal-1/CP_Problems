...
// Codeforces: https://codeforces.com/profile/n___er
...
int fun(string a,string b,vector<vector<int>> &dp,int x,int y,string path,string ans){
    if(x == a.size() && y == b.size()-1){
        ans += b[y];
        return path == ans ? 1:0;
    }
    // if(dp[x][y] != -1){
    //     return dp[x][y];
    // }
    int n = fun(a,b,dp,x+1,y+1,path,ans += a[x]);
    int m = fun(a,b,dp,x+1,y+1,path,ans+=a[y]);
...
void solve()
{
    // code -->
    inint(x);
    // vector<vector<int>> veci(2,vector<int>(x,0));
    instr(a);
    instr(b);
    string path = "";
    path += (a[0]);
    feach(i,0,x-1,1){
        char maxi = min(a[i+1],b[i]);
        path += maxi;
    }
    path += b[x-1];
    out(path);
    vector<vector<int>> dp(x,vec(x+1,-1));
    string ans ="";
    out(fun(a,b,dp,1,0,path,ans));
    // return f(veci,x,y,path);
}   
