...
int solve(int n, int m, int ix, int iy, int tx, int ty, int d) {
    int dis = abs(tx - ix) + abs(ty - iy);
    if (dis <= d) {
        return INT_MAX;
    }
    if (ix == n - 1 && iy == m - 1) {
    ...
    if (ix < 0 || ix >= n || iy >= m || iy < 0) {
        return INT_MAX;
    }

    int a = solve(n, m, ix + 1, iy, tx, ty, d);
    int b = solve(n, m, ix, iy + 1, tx, ty, d);
...
void solve()
{
    ...
    int a = solve(veci[0],veci[1],0,0,veci[2]-1,veci[3]-1,veci[4]);
    out(a);
}
...
int32_t main()
{
    ...
    #endif
    //              ☆*: .｡. o(≧▽≦)o .｡.:*☆
            //    coding karne ke liya skills (ham pe to haye nahi)  
    int t = 1;
    cin>>t;
    while(t--)
    {
