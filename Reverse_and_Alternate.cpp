...
bool f(string st,ll N,int tp){
    string tempp;
    rep(i,N) tempp += (i%2 ? (tp==0?'1':'0') : tp+'0');
    vec veci;
    rep(i,N)
    { 
        if(st[i]!=tempp[i]) veci.pb(i);
    }
    if(veci.empty()){
        return true;
    }
    string rev_st = st;
    reverse(rev_st.begin()+veci.front(), rev_st.begin()+veci.back()+1);
    return rev_st == tempp;
}
void ujjwal(){
    // Code Starts Here
