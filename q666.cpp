...
int countLatticePoints(ll x) {
    ll cnt = 0;
    for (ll i = x - 1; i >= 1; i--) {
        double dist = sqrt(i * i + x * x);
        if (dist >= x && dist < x + 1) {
            cnt++;
        }
    }
    cnt *= 2;
    double diagonalDist = sqrt(2) * x;
    if (diagonalDist >= x && diagonalDist < x + 1) {
        cnt++;
    }
    cnt *= 4;
    cnt += 4;
...
void solve()
{
    ...
    // ll rows = x+1;
    ll cnt = 0;
    int left = -x, right = x, top = x, bottom = -x;
        
        while (left <= right && top <= bottom) {
            if(left == x-1 && right ==  x-1 &&  top == x-1 && bottom == x-1){
                break;
            }
