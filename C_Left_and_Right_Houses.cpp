...
int fun(vec &nums, int k,mp &hash,int ind,vec val){
    if(ind >= nums.size()){
        return 0 ;
    }
    if(k == 0){
        out(val);
        return abs(hash.begin()->f - hash.rbegin()->f);
    }
    // not take
    ll a = fun(nums,k,hash,ind+1,val);
    //take 
    hash[nums[ind]]++;
    val.pb(nums[ind]);
    ll b = fun(nums,k-1,hash,ind+1,val);
    hash[nums[ind]]++;
    val.pop_back();
    if(hash[nums[ind]] == 0){
        hash.erase(nums[ind]);
    ...
    int sumOfPowers(vec &nums,ll k) {
        cy;
        ll  ans = 0;
        mp hash;
        ll ind =0;
        vec val;
        return fun(nums,k,hash,ind,val);
    }
void solve()
{
    inll(x);
