...
void solve()
{
    ...
    cin>>a>>b;
    // fr (i,0,n+1) cin>>b[i];
    ll pp=0, mini=INT_MAX;
    ll last=b[n];
    bool flag=false;
    rep(i,n) {
        pp+=abs(a[i]-b[i]);
        if (last>=a[i] && last<=b[i] || last>=b[i]&&last<=a[i]) flag=true;
        mini = min({(ll)abs(last - a[i]),(ll)abs(b[i]-last) ,mini});
    }
    if (flag==true) cout<<pp+1<<endl;
    else cout<<pp+mini+1<<endl;
}   
