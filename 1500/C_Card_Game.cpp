...
void f(ll n){
    ...
    // iterating towards the back to find the answer to the minimum Queries.
    f(n-2);
    // What does below statements say ? 
    // dp[n][0] state the number of cases alice will win is the case when Alice have nth card then he will always win the game + some case
    // somecase are the case when bob
...
void ujjwal(){
    ...
    dp[2][2] = 1;
    f(x);
    cout<<dp[x][0] <<" "<<dp[x][1]<<" "<<dp[x][2]<<endl;;
}
