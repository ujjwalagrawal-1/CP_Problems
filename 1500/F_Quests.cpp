...
bool f(int mid,vecl &veci,vecl& prexsum,int b,int td){
    ll cnt = td/(mid+1);

    return cnt*prexsum[mid] + prexsum[(td/(mid+1))-1]>= b;
}
void ujjwal(){
    // Code Starts Here
    ll a,b,c;
    ...
    vecl veci(a);
    cin>>veci;
    sort(all(veci),greater<ll>());
    vector<ll> prexsum(a);
    prexsum[0] = veci[0];
    feach(i,1,a,1){
        prexsum[i] = prexsum[i-1] + veci[i];
