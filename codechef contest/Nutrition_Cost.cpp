...
void ujjwal(){
    ...
    vecl a(x),b(x);
    cin>>a>>b;
    // what we wwill do is we will take minimu cost for the every vitamin possible 
    // so for this we will create a map data structure in which we will store the minimum cost food availble for it;
    mpl mp;
    for(int i = 0;i<x;i++){
        if(mp.count(a[i])){
    ...
    ll cnt = 0;
    for(auto itr : mp){
        if(y*(cnt+1) - (sum + itr.second) > maxi){
            sum += itr.second;
            maxi = max(maxi,y*(cnt+1) - sum);
            cnt++;
        }
    }
