...
void solve()
{
    ...
    ll maxi  = INT_MIN;
    feach(i,1,x-1,1){
        if(veci[i] < veci[i-1] && veci[i] < veci[i + 1]){
            maxi = max(maxi,veci[i-1]-veci[i]); mini = min(mini,veci[i+1]-veci[i]);
        }
        else if(veci[i] < veci[i-1] && veci[i] > veci[i + 1]){
            out(0);
            rtn;
        }
    }
    if(veci[x-2] > veci[x-1]){
        maxi = max(maxi,veci[x-2]- veci[x-1]);mini = min(mini,x);
    ...
    rep(i,sz(val)){
        mini = min(mini,val[i][1]); 
        maxi = max(maxi,val[i][0]);
    }
